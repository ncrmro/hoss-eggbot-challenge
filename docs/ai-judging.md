...
- **No custom scripts (preferred):** the official [`actions/ai-inference`](https://github.com/actions/ai-inference) action wraps this API nicely but has **no image support upstream**. We've prototyped an `image-file` input extension (see [patches/ai-inference-image-file-input.patch](patches/ai-inference-image-file-input.patch)) that attaches images to the prompt as base64 `image_url` content parts — verified against the upstream test suite (114 tests passing). Plan: publish it from a fork (e.g. under an `ai-outfitter` profile/org) as `ai-outfitter/ai-inference@image-file`, and ideally upstream it as a PR to `actions/ai-inference`. With that in place, each matrix judge is a single `uses:` step with `image-file: judged/<team>.jpg` — no `judge.py` needed; only the tiny collect/aggregate glue remains.
